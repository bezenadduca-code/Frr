...
local function docRemoveESP()
    if docCurrentESP and docCurrentESP.Parent then
        docCurrentESP:Destroy()
    end
    if docCurrentBillboard and docCurrentBillboard.Parent then
        docCurrentBillboard:Destroy()
    end
    docCurrentESP = nil
...
svc.WS.DescendantAdded:Connect(function(v)
    if not docESPEnabled then return end
    task.wait(0.1)
    if docESPEnabled and not docCurrentESP then
        docScan(v)
    end
end)
