-- BetrayalHub - Complete Fixed Version with Standalone Aimbot
...
-- ============================================================
-- AIMBOT TAB - LOAD STANDALONE AIMBOT
-- ============================================================
...
-- Track loaded aimbot state
local aimbotLoaded = false
local aimbotThread = nil
local aimbotConnections = {}

-- The standalone aimbot code as a string
local AIMBOT_SCRIPT = [[
-- ============================================================
-- ENSTRANGLE AIMBOT - TRUE LOCK-ON
-- ============================================================
    ...
    for _, model in ipairs(survivors:GetChildren()) do
        if model ~= char and model:IsA("Model") then
            ...
            if r and h and h.Health > 0 then
                ...
                if d < bestDist and d <= settings.maxDistance then
                    bestDist = d
                    best = r
                end
            end
...
-- Get head position with prediction
local function getSurvivorHeadPos(targetHRP)
    if not targetHRP or not targetHRP.Parent then return nil end
    local head = targetHRP.Parent:FindFirstChild("Head")
    local pos = head and head.IsA(head, "BasePart") and head.Position or (targetHRP.Position + Vector3.new(0, 2.5, 0))
    if targetHRP.AssemblyLinearVelocity then
        local velocity = targetHRP.AssemblyLinearVelocity
        local speed = velocity.Magnitude
        local pred = speed > 55 and 0.35 or speed > 30 and 0.22 or 0.12
        pos = pos + (velocity * pred)
    end
    return pos
end

-- Lock camera
local function trueLockOnCamera(survivorHeadPos)
    local char = lp.Character
    ...
    if distance > 0.01 then
        ...
        local targetCameraPos = hrp.Position + dirFromSurvivor * 5 + Vector3.new(0, 3, 0)
    ...
    pcall(function()
        ...
        if lookDir.Magnitude > 0.01 then
            ...
            hrp.CFrame = hrp.CFrame:Lerp(CFrame.new(hrp.Position, hrp.Position + lookDir), 0.6)
        end
    end)
...
local function startLock(targetHRP)
    ...
    state.locked = true
    state.lockConnection = RunService.RenderStepped:Connect(function()
        if not state.locked or not state.target or not state.target.Parent then
            unlockTarget()
            return
        end
        local hum = state.target.Parent:FindFirstChildOfClass("Humanoid")
        if not hum or hum.Health <= 0 then
            unlockTarget()
            return
        end
        local headPos = getSurvivorHeadPos(state.target)
        if headPos then trueLockOnCamera(headPos) end
    end)
end
        ...
        if settings.enabled then
            task.spawn(function()
                ...
                if abilityName:lower():find("slash") then
                    if settings.slashUnlock then
                        unlockTarget()
                        return
                    end
                end
                local target = getNearestSurvivor()
                if target then startLock(target) end
            end)
        end
    end
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == Enum.KeyCode.L then
        ...
        if not settings.enabled then unlockTarget() end
    end
    if input.KeyCode == Enum.KeyCode.U then unlockTarget() end
end)
...
_G.ENSTRANGLE_AIMBOT = {
    settings = settings,
    state = state,
    unlockTarget = unlockTarget,
    startLock = startLock,
    getNearestSurvivor = getNearestSurvivor,
}
]]

-- Load function
local function loadAimbot()
    if aimbotLoaded then return true end
    
    print("[BetrayalHub] Loading Enstrangle aimbot...")
    ...
    -- Execute the script
    local success, err = pcall(loadstring, AIMBOT_SCRIPT)
    if not success then
        print("[BetrayalHub] Failed to compile aimbot: " .. tostring(err))
        return false
    end
    ...
    if not execSuccess then
        print("[BetrayalHub] Failed to execute aimbot: " .. tostring(execErr))
        return false
    end
    
    aimbotLoaded = true
    print("[BetrayalHub] ✅ Standalone aimbot loaded")
    
    -- Create a status check loop
    local statusThread = task.spawn(function()
        while aimbotLoaded do
            task.wait(0.5)
            pcall(function()
                if _G.ENSTRANGLE_AIMBOT then
                    local s = _G.ENSTRANGLE_AIMBOT.settings
                    local st = _G.ENSTRANGLE_AIMBOT.state
                    if s and st then
                        local status = s.enabled and "🟢 ON" or "⚪ OFF"
                        local locked = st.locked and "🔒 Locked" or "🔓 Unlocked"
                        local target = st.target and st.target.Parent and st.target.Parent.Name or "None"
                        if statusPara then
                            statusPara:Set({ 
                                Title = "Status", 
                                Desc = "Loaded: ✅\nAimbot: " .. status .. "\nLock: " .. locked .. "\nTarget: " .. target
    ...
    table.insert(aimbotConnections, {Disconnect = function() task.cancel(statusThread) end})
...
local function unloadAimbot()
    if not aimbotLoaded then return end
    
    print("[BetrayalHub] Unloading Enstrangle aimbot...")
    
    -- Unlock target
    pcall(function()
        if _G.ENSTRANGLE_AIMBOT then
            _G.ENSTRANGLE_AIMBOT.unlockTarget()
            _G.ENSTRANGLE_AIMBOT.settings.enabled = false
        end
    end)
    ...
    if statusPara then
        statusPara:Set({ 
            Title = "Status", 
            Desc = "Aimbot not loaded\nToggle 'Load Aimbot' to enable" 
        })
    end
    
    print("[BetrayalHub] ❌ Standalone aimbot unloaded")
end

-- Toggle function
local function toggleAimbot(load)
    if load then
        loadAimbot()
    else
        unloadAimbot()
...
aimSec:Toggle({
    Title = "Load Aimbot",
    Desc = "Load/unload standalone Enstrangle aimbot",
    Default = false,
...
-- Status paragraph
local statusPara = aimSec:Paragraph({ 
    Title = "Status", 
    Desc = "Aimbot not loaded\nToggle 'Load Aimbot' to enable" 
})

-- Keybinds for the toggle (only work when loaded)
svc.Input.InputBegan:Connect(function(input, processed)
    if processed then return end
    
    -- L key to toggle aimbot on/off (only if loaded)
    if input.KeyCode == Enum.KeyCode.L then
        if aimbotLoaded and _G.ENSTRANGLE_AIMBOT then
            local s = _G.ENSTRANGLE_AIMBOT.settings
            if s then
                s.enabled = not s.enabled
                if not s.enabled then
                    pcall(_G.ENSTRANGLE_AIMBOT.unlockTarget)
                end
            end
        end
    end
    
    -- U key to unlock (only if loaded)
...
-- Unload aimbot when tab is destroyed or hub closes
local function cleanupAimbot()
    unloadAimbot()
end

-- Hook into tab destruction
local origDestroy = tabAimbot.Destroy
tabAimbot.Destroy = function()
    cleanupAimbot()
    if origDestroy then origDestroy(tabAimbot) end
end

-- Also clean up on window close
pcall(function()
    if win then
        local origWinDestroy = win.Destroy
        win.Destroy = function()
            cleanupAimbot()
            if origWinDestroy then origWinDestroy(win) end
        end
    end
end)

print("[BetrayalHub] Aimbot tab ready - toggle to load standalone aimbot")
...
-- ============================================================
-- GROUNDBULB & VINE ESP (PURPLE - OVERRIDES GAME HIGHLIGHTS)
-- ============================================================
...
print("[GV ESP] Loaded - Purple highlights")
