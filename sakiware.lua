...
local function espDocumentMatch(obj)
    if not obj:IsA("MeshPart") then return false end
    local prompt = obj:FindFirstChildOfClass("ProximityPrompt")
    if not prompt then return false end
    local txt = ((prompt.ActionText or "") .. " " .. (prompt.ObjectText or "")):lower()
    return txt:find("collect") ~= nil
end
local function espDoDocument(on)
    for _, obj in ipairs(svc.WS:GetDescendants()) do
        if espDocumentMatch(obj) then
            if on then espAttach(obj, "esp_doc", Color3.fromRGB(255, 255, 0), false)
            else espDetach(obj, "esp_doc") end
        end
    end
end
...
local function espBindWorld()
    ...
    table.insert(esp.mapConns, svc.WS.DescendantAdded:Connect(function(obj)
        task.wait(0.2)
        if esp.items and (obj.Name == "BloxyCola" or obj.Name == "Medkit") then
            if obj and obj.Parent and not espItemHeld(obj) then espAttach(obj,"esp_i",espItemColor(obj.Name),false) end
        end
        if esp.document and obj and obj.Parent and espDocumentMatch(obj) then
            espAttach(obj, "esp_doc", Color3.fromRGB(255, 255, 0), false)
        end
    end))
...
secESP:Toggle({ Title="Document",   Type="Checkbox", Flag="espDocument",   Default=esp.document,   Callback=function(on) esp.document=on;   task.spawn(function() espDoDocument(on)   end) end })
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        if esp.items      then task.spawn(function() espDoItems(true)      end) end
        if esp.buildings  then task.spawn(function() espDoBuildings(true)  end) end
        if esp.document   then task.spawn(function() espDoDocument(true)   end) end
