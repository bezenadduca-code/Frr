...
-- HDT: Camera flick toward killer so the player's own movement input walks into the hitbox.
-- No BodyVelocity, no MoveTo — camera snaps to face killer, character follows naturally.
local function combatHDTBeginDrag(killerModel)
    ...
    local toKiller = (targetHRP.Position - hrp.Position) * Vector3.new(1, 0, 1)
    if toKiller.Magnitude < 0.01 then _combatHDTDebounce = false; return end

    local dir    = toKiller.Unit
    -- Place camera behind player relative to killer direction, looking at player head
    local camPos = hrp.Position + Vector3.new(0, 2, 0) - dir * 8

    cam.CameraType = Enum.CameraType.Scriptable
    cam.CFrame     = CFrame.new(camPos, hrp.Position + Vector3.new(0, 1, 0))

    -- Restore after duration — player's own movement input does the rest
    local duration = combatS.hdtDelay > 0 and (combatS.hdtDelay + 0.25) or 0.3
